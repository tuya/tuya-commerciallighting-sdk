package com.thingclips.smart.commercial.lighting.demo.widget.recyclerview.adapter;
...
import com.thingclips.smart.commercial.lighting.demo.widget.recyclerview.item.BaseFoot;
import com.thingclips.smart.commercial.lighting.demo.widget.recyclerview.item.BaseHead;
...
class SectionList<I extends BaseItem> implements Cloneable {
    ...
    public int setItemViewList(@IntRange(from = 0) int key, @NonNull List<I> itemList) {
        Log.d(TAG, "Stat to add the itemList, itemList size: " + itemList.size());
    ...
    public int clearAllItemView(@IntRange(from = 0) int key) {
        Section section = findSectionByKey(key);
        if (null == section) {
            Log.w(TAG, "Can't find the section by key: " + key);
            return -1;
        }

        int size = section.getItemCount();
    ...
    public boolean itemViewMove(int fromPosition, int toPosition) {
        Log.d(TAG, "Move the item from:" + fromPosition + " to " + toPosition);
    ...
    public int clearAllHeadView(@IntRange(from = 0) int key) {
        Section section = findSectionByKey(key);
        if (null == section) {
            Log.w(TAG, "Can't find the section by key: " + key);
            return -1;
        ...
        int size = section.getHeadCount();
        section.getHeadViewList().clear();
    ...
    public void clearAllSectionHeadView() {
        for (int index = 0; index < mSectionList.size(); index++) {
            int oldSize = mSectionList.valueAt(index).clearHeadViewList();
            int sectionKey = mSectionList.valueAt(index).getSectionKey();
    ...
    public int getCount() {
        ...
        Log.d(TAG, "The sectionList count: " + count);
        return count;
    }
    ...
    public int getViewType(@IntRange(from = 0) int position) {
        Section section = findSection(position);
        if (null == section) {
            Log.w(TAG, "Can't find the section by position: " + position);
            return -1;
        }
    ...
    public SparseArrayCompat<BaseHead> getHeadViewList(@IntRange(from = 0) int key) {
        if (null == this.mSectionList.get(key)) {
            Log.w(TAG, "The itemView list is empty.");
            return null;
        }

        return this.mSectionList.get(key).getHeadViewList();
    }
    ...
    public I getItemViewByViewType(int viewType) {
        Log.d(TAG, "Start to find ItemView by viewType: " + viewType);
        Section section;
        for (int index = 0; index < this.mSectionList.size(); index++) {
    ...
    public BaseFoot getFootViewByViewType(int viewType) {
        if (!isFootViewViewType(viewType)) {
            Log.w(TAG, "The viewType is not a foot viewType. viewType: " + viewType);
            return null;
        }
    ...
    private class Section {
        ...
        public int clearHeadViewList() {
            int oldSize = this.mHeadViewList.size();
